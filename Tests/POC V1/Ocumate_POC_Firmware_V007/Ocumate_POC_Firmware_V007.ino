...
#define DROPS_BASELINE_INDEX 1
#define DROPS_THRESHOLD_HIGHER_INDEX 2
#define DROPS_THRESHOLD_LOWER_INDEX 3
...
double medCal[NUM_MEDS][8] = {
                  {145,92,0.2,0.1,500,80,90,8},  // Combigan
                  {87,82,0.3,-0.3,500,82,90,7},  // Xalacom - 45
                  {145,75,0.5,-0.5,500,80,90,8}   // Latanoprost
                  };
...
void loop() {
  ...
  dropCharacteristic.writeValue(dropCount);  // Broadcast number of detected drops
  dropsLeftCharacteristic.writeValue(medCal[medSelect][DROPS_MAX_INDEX] - dropCount);
  ...
  switch (state) { 
    ...
    case DISPENSING:
      ...
      if (derivative_mean > medCal[medSelect][DROPS_THRESHOLD_HIGHER_INDEX]) { 
        Serial.print("***------- ");
        Serial.print(derivative_mean);
        Serial.print(", ");
        Serial.print(medSelect);
        Serial.print(", ");
        Serial.print(DROPS_THRESHOLD_HIGHER_INDEX);
        Serial.print(", ");
        Serial.print(medCal[medSelect][DROPS_THRESHOLD_HIGHER_INDEX]);
        Serial.println(" -------***");
