...
// TO-DO
// * Pre-load actuator to appropriate start position based on selected bottle
// * Store relative number of drops in matrix for each bottle
// * Pause dispensing if not at the correct angle
...
// Drops per Bottle | Baseline | High Threshold | Low Threshold | Max Suspension Time | Pitch Min Limit | Pitch Max Limit | Roll Limit
#define NUM_MEDS 3
#define EPS_MED_SELECT 4
#define DROPS_MAX_INDEX 0
#define NUM_DROPS_INDEX 1
#define DROPS_BASELINE_INDEX 2
#define DROPS_THRESHOLD_HIGHER_INDEX 3
#define DROPS_THRESHOLD_LOWER_INDEX 4
#define MAX_SUSPENSION_TIME_INDEX 5
#define PITCH_MIN_LIMIT_INDEX 6
#define PITCH_MAX_LIMIT_INDEX 7
#define ROLL_MAX_LIMIT_INDEX 8

// #define DROP_THRESH_UPPER 0.2 // LTN0
// #define DROP_THRESH_LOWER -0.2 // LTN0
#define DROP_THRESH_UPPER 0.3 // CMB0
#define DROP_THRESH_LOWER 0.1 // CMB0
...
movingAvg p2MovingAvg(MOVING_AVG_SAMPLES);
...
unsigned long curr_millis, prev_millis;
...
BLEByteCharacteristic resetCharacteristic("20B10028-E8F2-537E-4F6C-D104768A1214", BLERead | BLEWrite);        // Resets the device
...
//                   {145,92,0.3,0.1,500,80,90,8},  // Combigan
//                   {87,82,0.3,-0.3,500,82,90,7},  // Xalacom - 45
//                   {145,75,0.2,-0.2,500,80,90,8}   // Latanoprost
//                   };
...
void setup() {
  ...
  mean = p2MovingAvg.reading(proximity);
}
...
void loop() {
  ...
  medicationCharacteristic.writeValue(medSelect);  // Broadcast number of detected drops
  batteryLevelCharacteristic.writeValue(batteryLevel);
  dropCharacteristic.writeValue(medCal[medSelect][NUM_DROPS_INDEX]);  // Broadcast number of detected drops
  dropsLeftCharacteristic.writeValue(medCal[medSelect][DROPS_MAX_INDEX] - medCal[medSelect][NUM_DROPS_INDEX]);
  ...
  double derivative_mean = (mean - prev_mean) / (float) (curr_millis - prev_millis + 0.01); // +1 avoids db0 error
  ...
  if (printIR) {
    ...
    Serial.print(",");
    Serial.println(derivative_mean);
  ...
  switch (state) { 
    ...
    case DISPENSING:
      ...
      if (derivative_mean > medCal[medSelect][DROPS_THRESHOLD_HIGHER_INDEX]) { 
        
        // DEBUG
        Serial.println("***------- " + String(derivative_mean) + ", " + String(medSelect) + ", " + String(DROPS_THRESHOLD_HIGHER_INDEX) + ", " + String(medCal[medSelect][DROPS_THRESHOLD_HIGHER_INDEX]) + " -------***");
    ...
    case SUSPENDED: 
      ...
      if (derivative_mean < medCal[medSelect][DROPS_THRESHOLD_LOWER_INDEX]) { 
        state = IDLE;
        // dropCount++;
        medCal[medSelect][NUM_DROPS_INDEX]++;
        tocDrop = millis();
        if (usbConnected) {
...
void medicationCharacteristicWritten(BLEDevice central, BLECharacteristic characteristic) {
  ...
  medSelect = characteristic.value()[0]-1;
  Serial.println(medSelect);
  medSelectLED();
...
void checkBatteryLevel() {
  ...
  batteryLevel = (voltage-minVoltage) / (maxVoltage-minVoltage)*100;
